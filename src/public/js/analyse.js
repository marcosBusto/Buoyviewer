...
const chart5 = sdk5.createChart({ chartId: '63c308ef-2549-47e5-8109-cbc2ada8c84f' });
...
const chart6 = sdk6.createChart({ chartId: '63c2fb22-2549-4de9-841b-cbc2ad9e4947' });
...
$(document).ready(function() {
  ...
  $.each(temp, function(id, name) {
    $select.append('<option value=' + name.id + '>' + name.cruise + '</option>');
  });
});

const cruiseSelect = document.getElementById("cruise");
cruiseSelect.addEventListener("change", async (e) => {
  if(cruiseSelect.value == "") {
    await chart1.setFilter({});
    await chart2.setFilter({});
    await chart3.setFilter({});
    await chart4.setFilter({});
    await chart5.setFilter({});
    await chart6.setFilter({});
  } else {
    const cruiseValue = parseInt(cruiseSelect.value);
    await chart1.setFilter({ cruise:cruiseValue });
    await chart2.setFilter({ cruise:cruiseValue });
    await chart3.setFilter({ cruise:cruiseValue });
    await chart4.setFilter({ cruise:cruiseValue });
    await chart5.setFilter({ cruise:cruiseValue });
    await chart6.setFilter({ cruise:cruiseValue });
  }
});